...
// Initialize controller and load resource
require_once __DIR__ . '/../../app/controllers/AdminResourceController.php';
$controller = new AdminResourceController();
$resource = $controller->edit($resourceId);

if (!$resource) {
    // Controller handles redirect for not found resources
    exit;
}
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Update mental health resource information</p>
        </div>
        ...
        <div class="main-content">
            ...
            <div class="metadata-info">
                <strong>📅 Created:</strong> <?= date('F j, Y g:i A', strtotime($resource['created_at'])) ?> |
                <strong>📝 Last Updated:</strong> <?= date('F j, Y g:i A', strtotime($resource['updated_at'])) ?> |
                <strong>👁️ Views:</strong> <?= number_format($resource['view_count'] ?? 0) ?>
            </div>
            ...
            <form id="resource-form" method="POST" action="/MindDB/public/admin_resource_action.php" class="form-container">
                    ...
                    <div class="form-group">
                        ...
                        <select id="resource_type" name="resource_type" class="form-select" required>
                            ...
                            <?php foreach ($controller->getResourceTypes() as $key => $label): ?>
                                <option value="<?= $key ?>" <?= $resource['resource_type'] === $key ? 'selected' : '' ?>>
                ...
                <div class="form-group full-width">
                    ...
                    <?php if ($resource['url']): ?>
                        <div class="url-preview">
                            <strong>Current URL:</strong> <a href="<?= htmlspecialchars($resource['url']) ?>" target="_blank" style="color: #007bff;"><?= htmlspecialchars($resource['url']) ?></a>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-group full-width">
                    ...
                    ><?= htmlspecialchars($resource['description'] ?? '') ?></textarea>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            name="category" 
                            class="form-input" 
                            value="<?= htmlspecialchars($resource['category'] ?? '') ?>"
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            name="author" 
                            class="form-input" 
                            value="<?= htmlspecialchars($resource['author'] ?? '') ?>"
                    ...
                    <div class="form-group">
                        ...
                        <select id="difficulty_level" name="difficulty_level" class="form-select">
                            ...
                            <?php foreach ($controller->getDifficultyLevels() as $key => $label): ?>
                                <option value="<?= $key ?>" <?= $resource['difficulty_level'] === $key ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($label) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
